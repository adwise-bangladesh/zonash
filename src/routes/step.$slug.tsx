      ...
      <section className="mt-8 px-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2.5">
          <a
            ...
            className="group flex items-center justify-center gap-2 rounded-[6px] border border-border bg-background px-3 py-3 text-sm font-bold text-foreground transition-colors hover:border-primary hover:text-primary"
            aria-label="Call to order"
          >
            <span className="grid h-8 w-8 place-items-center rounded-full bg-primary/10 text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground">
          ...
          <a
            href={`https://wa.me/8801926644575?text=${encodeURIComponent(
              `Hi Zonash, I'd like to order:\n\n${product.name}${selectedVariation ? ` — ${selectedVariation.attributes.map((a) => a.option).join(" / ")}` : ""}\nPrice: ${formatBDT(unitPrice)}\n\nLink: ${product.permalink}`,
            )}`}
            target="_blank"
            rel="noopener noreferrer"
            className="group flex items-center justify-center gap-2 rounded-[6px] border border-[#25D366]/40 bg-[#25D366]/[0.06] px-3 py-3 text-sm font-bold text-[#128C7E] transition-colors hover:bg-[#25D366] hover:text-white hover:border-[#25D366]"
            aria-label="Order on WhatsApp"
          >
            <span className="grid h-8 w-8 place-items-center rounded-full bg-[#25D366] text-white">
              <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor" fillRule="evenodd" clipRule="evenodd" aria-hidden>
