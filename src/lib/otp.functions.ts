  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const res = await sendSms({
        phone,
        message: `Zonash: Your order #${created.number} verification code is ${code}. Valid 5 minutes. Do not share.`,
      });
      smsOk = res.ok;
...
export const resendOrderOtp = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const res = await sendSms({
        phone: r.phone,
        message: `Zonash: Your order #${data.order_id} verification code is ${code}. Valid 5 minutes.`,
      });
      smsOk = res.ok;
    } catch (e) {
